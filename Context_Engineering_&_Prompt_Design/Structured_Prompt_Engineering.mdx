...
**Basic Template:**
[Full runnable structured prompt notebook](https://colab.research.google.com/drive/15hKw-5PONe0DKkDmYVfLviFJUoK3aBcn?usp=drive_link)
<CodeGroup>
```python Python
prompt = """
[ROLE]
...
[CONTEXT]
AcmeCo sells software subscriptions. Our refund policy: 30-day money-back guarantee, 
no questions asked. After 30 days, refunds require manager approval.

[INSTRUCTIONS]
Analyze the customer message below and:
1. Identify the customer's primary issue
2. Determine if this qualifies for automatic refund
...
**In Production:**
- Cost impact: Clear output schemas reduce parsing errors and retries (5–15% fewer re-runs).
- Reliability: Structured prompts are easier to validate and monitor.
- Performance: Slight token overhead; mitigate with caching (see 5.2).
...
- 40-60% reduction in hallucinations in controlled studies
...
Background context: ${knowledgeBase}
Previous conversation: ${history}
`;
// LLM might confuse where context ends and question begins
```
...
**In Production:**
**Cost Impact:** Tagging adds tokens; mitigate with caching (50–90% on static parts).
...
**Pattern:**
[Full runnable few-shot examples notebook](https://colab.research.google.com/drive/1dCxVNAqgUY8bTiLXv32dvH5NyHSfZ67r?usp=drive_link)
<CodeGroup>
```python Python
prompt = f"""
...
**Production Note:**
Store examples separately and inject only relevant ones:
...
**Pattern:**
[Full runnable structured outputs with JSON schemas notebook](https://colab.research.google.com/drive/1ivZiyimkc16BkvUYqEKtmd4xuvpLF8aU?usp=drive_link)
<CodeGroup>
```python Python
import jsonschema
...
function validateOutput(jsonText: string): any {
...
**Vendor features to explore:**
- OpenAI structured outputs with `response_format` (JSON schema)
- Anthropic tool-use/structure guidance
