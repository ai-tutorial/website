...
async function handleQuery(userId: string, query: string): Promise<string> {
    const promptVersion = getPromptVersion(userId);
    const promptTemplate = loadPrompt(promptVersion);
    
    const response = await llm.generate(promptTemplate.replace("{query}", query));
