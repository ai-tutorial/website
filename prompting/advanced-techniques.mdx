...
<Quiz>
<QuizQuestion question="You add 'think step by step' to a simple factual lookup ('What's the capital of France?'). What happens?" options={["Accuracy improves significantly", "You waste tokens — CoT helps complex reasoning, not simple recall", "The model refuses to answer"]} answer={1} explanation="CoT adds value for multi-step reasoning (math, logic, analysis). For simple factual retrieval, it just burns tokens without improving accuracy." />
<QuizQuestion question="Your prompt chain has 4 steps. Step 3 fails 20% of the time. What's the best fix?" options={["Add retry logic for step 3 only, without re-running the entire chain", "Combine all 4 steps into one prompt to avoid the failure point", "Lower the temperature globally across all steps"]} answer={0} explanation="Prompt chaining's key advantage: you can retry or fix individual steps. Combining everything into one prompt makes the whole thing fragile and harder to debug." />
<QuizQuestion question="You run self-consistency with 5 samples and get: 'Paris, Paris, Lyon, Paris, Marseille'. The majority answer is Paris. Should you trust it?" options={["Yes — 3/5 agreement is strong enough for production", "It depends — 60% agreement is moderate; check if the dissenting answers reveal ambiguity in the question itself", "No — you need at least 80% agreement"]} answer={1} explanation="Low agreement (60%) can signal an ambiguous question or insufficient context. Before trusting majority vote, investigate why the model produced different answers." />
</Quiz>
